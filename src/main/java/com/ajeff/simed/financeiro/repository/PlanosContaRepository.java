...
public interface PlanosContaRepository extends JpaRepository<PlanoConta, Long>, PlanosContaRepositoryQueries{
	...
	@Query("select p from PlanoConta p order by p.nome asc")
	List<PlanoConta> findAllOrderByNome();

	@Query("select p from PlanoConta p where p.tipo = 'DÉBITO' order by p.nome asc")
	List<PlanoConta> listarTodosPlanosContaDebito();

	@Query("select p from PlanoConta p where p.tipo = 'CRÉDITO' order by p.nome asc")
	List<PlanoConta> listarTodosPlanosContaCredito();

	@Query("select p from PlanoConta p where p.tipo = 'DÉBITO' and p.id = ?1 order by p.nome asc")
